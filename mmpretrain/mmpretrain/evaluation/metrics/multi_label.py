...
class MultiLabelMetric(BaseMetric):
    ...
    **Support** is the number of samples:
    ...
    def __init__(self,
                 ...
                 items: Sequence[str] = ('precision', 'recall', 'f1-score', 'accuracy'),
        ...
        for item in items:
            assert item in ['precision', 'recall', 'f1-score', 'support', 'accuracy'], \
    ...
    def compute_metrics(self, results: List):
        ...
        def pack_results(precision, recall, f1_score, support, accuracy):
            single_metrics = {}
            ...
            if 'accuracy' in self.items:
                single_metrics['accuracy'] = accuracy
    ...
    def calculate(
        ...
        _, top_k_indices = pred.topk(topk, dim=1)
        true_labels = torch.argmax(target, dim=1, keepdim=True)
        correct_predictions = torch.sum(top_k_indices == true_labels, dim=1)
        top_k_accuracy = torch.mean(correct_predictions.float())
        ...
        return *_precision_recall_f1_support(pos_inds, target, average), top_k_accuracy
