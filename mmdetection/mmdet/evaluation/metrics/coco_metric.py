    ...
    def results2json(self, results: Sequence[dict],
        ...
        for idx, result in enumerate(results):
            ...
            for i, label in enumerate(labels):
                ...
                if self.label2catid[label]==-1:
                    continue
                data['category_id'] = self.label2catid[label]
    ...
    def compute_metrics(self, results: list) -> Dict[str, float]:
        ...
        if self.cat_ids is None:
            ...
            for cls in self.dataset_meta['classes']:
                cat_id = self._coco_api.get_cat_ids(cat_names=[cls])
                if len(cat_id)==0:
                    label2catid.append(-1)
                elif len(cat_id)==1:
                    label2catid.append(cat_id[0])
                else:
                    raise NotImplementedError
            self.label2catid = label2catid
